...
import { searchJobs, analyzeJobMatch, getSearchQueryFromResume, SearchFilters, generateDorkFromResume, extractJobDetailsFromUrl } from '../services/gemini';
...
const MultiSelectDropdown = ({ 
  ...
  return (
    <div className="relative group">
      ...
      {isOpen && (
        <div className="absolute top-full left-0 mt-2 w-48 bg-zinc-900 border border-zinc-700 rounded-lg shadow-2xl z-20 overflow-hidden animate-in fade-in zoom-in-95 duration-100">
...
export const JobSearchSection: React.FC<JobSearchSectionProps> = ({ resumes, trackedJobs, onTrackJob, onMenuClick }) => {
  ...
  const [dorkResumeId, setDorkResumeId] = useState<string>(resumes[0]?.id || '');
  ...
  const handleSaveFilter = () => {
     if (Object.values(filters).every(v => Array.isArray(v) ? v.length === 0 : v === 'any')) {
         return; // Don't save empty filters
     }
     
     const name = prompt("Enter a name for this filter set:", "My Filter");
     if (!name) return;
  ...
  const performSearch = async (searchQuery: string) => {
    ...
    try {
      const foundJobs = await searchJobs(searchQuery, filters);
      ...
      for (const job of jobsWithState) {
        analyzeJobMatch(job, resumes).then(analysis => {
          setJobs(currentJobs => 
            currentJobs.map(j => 
              j.id === job.id 
                ? { ...j, analysis, isAnalyzing: false } 
                : j
            )
          );
        });
      ...
      console.error(err);
      if (err.message && err.message.includes("Automatic Job Search requires Google Grounding")) {
        setError(err.message);
      } else {
        setError("Something went wrong. Please check your API Settings.");
      }
      setIsSearching(false);
    }
  };
  ...
  const handleResumeSearch = async () => {
    ...
    try {
      const suggestedQuery = await getSearchQueryFromResume(resume);
      setQuery(suggestedQuery);
      setSearchMode('keyword'); 
      performSearch(suggestedQuery);
    } catch (e) {
      setError("Failed to analyze resume. Check Settings.");
    } finally {
  ...
  const handleGenerateDork = async () => {
      ...
      try {
          ...
          console.error(e);
          setGeneratedDork("Error generating dork.");
      } finally {
  ...
  const handleExtractFromUrl = async () => {
      ...
      try {
          const details = await extractJobDetailsFromUrl(urlToExtract);
          setManualJob({
              title: details.title,
          ...
          setEntryMode('manual'); // Switch to manual view to show results
      } catch (e: any) {
          console.error(e);
          setExtractionError(e.message || "Failed to extract details. Try manual entry.");
      } finally {
  ...
  const handleAnalyzeAndListManualJob = async () => {
      ...
      // Set job immediately as "analyzing"
      setJobs([{ ...tempJob, isAnalyzing: true }]);

      try {
          const analysis = await analyzeJobMatch(tempJob, resumes);
          setJobs([{ ...tempJob, analysis, isAnalyzing: false }]);
      } catch (e) {
          console.error(e);
          setJobs([{ ...tempJob, isAnalyzing: false }]); // Show without analysis on error
      } finally {
  ...
  return (
    <>
      ...
      {/* 
        Main Container: 
        Changed from flex-col splitting to a single relative container 
        that handles scrolling. This allows sticky positioning to work 
        naturally and content to scroll 'behind' the header.
      */}
      <div className="flex-1 h-full bg-black relative overflow-y-auto custom-scrollbar scroll-smooth">
          ...
          {activeTab === 'search' && (
            <div className="animate-in fade-in slide-in-from-top-2">
              ...
              {showFilters && searchMode !== 'manual' && (
                <div className="mb-4 animate-in fade-in slide-in-from-top-2 bg-zinc-900/50 p-4 rounded-xl border border-zinc-800/50 backdrop-blur-sm">
                  <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                    <div>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                          {filters.locations.map(loc => (
                            <span key={loc} onClick={() => toggleFilterArray('locations', loc)} className="cursor-pointer text-[10px] bg-zinc-800 border border-zinc-700 px-2 py-0.5 rounded-full text-zinc-300 hover:bg-red-900/30 hover:text-red-300 hover:border-red-900/30 transition-colors">
                              {loc} ✕
                            </span>
                          ))}
                      </div>
                    ...
                    <div className="relative">
                      <select
                        ...
                        className="w-full h-full px-3 py-2 text-sm bg-zinc-950 border border-zinc-700 rounded-lg text-zinc-300 focus:border-emerald-500/50 focus:ring-1 focus:ring-emerald-500/20 outline-none appearance-none transition-all"
                      >
                        <option value="any">Any Time</option>
                        <option value="past_30_min">Last 30 Mins</option>
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 pt-3 border-t border-zinc-800/50">
                        ...
                        {savedFilters.map(sf => (
                           ...
                           >
                              ...
                              <span 
                                ...
                                className="opacity-0 group-hover:opacity-100 p-0.5 hover:bg-red-900/50 hover:text-red-300 rounded"
                              >
                     ...
                     <div className="flex gap-2">
                         ...
                         <button 
                           ...
                           className="flex items-center gap-1.5 px-4 py-1.5 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 hover:text-white text-xs font-medium rounded-lg border border-zinc-700 hover:border-zinc-600 transition-colors disabled:opacity-50"
                         >
                ...
                <form onSubmit={handleKeywordSearch} className="relative max-w-4xl animate-in fade-in slide-in-from-left-2">
                  ...
                  <div className="absolute inset-y-1.5 md:inset-y-2 right-1.5 md:right-2 flex gap-1.5">
                    <button
                      ...
                      className="h-full px-3 md:px-4 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 hover:text-white border border-zinc-700 rounded-lg transition-colors flex items-center justify-center hover:shadow-md"
                    >
                ...
                <div className="max-w-4xl bg-zinc-900 border border-zinc-700 rounded-xl p-2 md:p-3 flex flex-col md:flex-row gap-3 animate-in fade-in slide-in-from-right-2 shadow-lg">
                  <div className="flex-1">
                    <select 
                      ...
                      className="w-full h-full bg-zinc-950 border border-zinc-700 rounded-lg px-3 py-3 md:py-2 text-zinc-200 focus:ring-1 focus:ring-emerald-500 outline-none hover:border-zinc-600 transition-colors cursor-pointer"
                    >
                      {resumes.map(r => (
                  ...
                  <button 
                    ...
                    className="px-6 py-3 md:py-2 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 text-white rounded-lg font-medium flex items-center justify-center gap-2 transition-all hover:shadow-lg shadow-emerald-900/20 whitespace-nowrap"
                  >
                    {isGeneratingQuery ? <Loader2 className="w-4 h-4 animate-spin" /> : <Sparkles className="w-4 h-4" />}
                    Auto-Search
                  </button>
                </div>
              )}
                      ...
                      <div className="relative z-10">
                        ...
                        <div className="space-y-4">
                            ...
                            <button 
                              ...
                              disabled={isGeneratingDork || !dorkResumeId}
                              className="w-full flex items-center justify-center gap-2 bg-zinc-800 hover:bg-zinc-700 border border-zinc-700 hover:border-zinc-600 text-white py-2.5 rounded-lg text-sm font-medium transition-all shadow-sm"
                            >
                          ...
                          {entryMode === 'auto' ? (
                               ...
                               <div className="p-6 bg-zinc-950/50 border border-dashed border-zinc-700 rounded-xl hover:border-emerald-500/50 transition-colors group">
                                  ...
                                  <div className="flex gap-2">
                                    ...
                                    <button 
                                      ...
                                      disabled={isExtracting || !urlToExtract}
                            ...
                            <div className="animate-in fade-in space-y-4">
                              ...
                              <div className="grid grid-cols-2 gap-4">
                                  <div className="space-y-1.5">
                                      <label className="block text-xs font-medium text-zinc-500 uppercase tracking-wider">Job Title</label>
                                      <input 
                                          type="text" 
                              ...
                              <div className="space-y-1.5">
                                  <label className="block text-xs font-medium text-zinc-500 uppercase tracking-wider">Job Description</label>
                                  <textarea 
                              ...
                              <div className="pt-2">
                                  <button 
                                    ...
                                    disabled={isAnalyzingManual || !manualJob.title || !manualJob.description}
                                    className="w-full flex items-center justify-center gap-2 bg-emerald-600 hover:bg-emerald-500 text-white py-3 rounded-lg text-sm font-medium transition-all hover:shadow-lg shadow-emerald-900/20 disabled:opacity-50"
                                  >
        ...
        <div className="p-4 md:p-8 pb-24 md:pb-24">
            ...
            <div className="flex flex-col items-center justify-center text-zinc-600 opacity-60 mt-10 md:mt-20">
               ...
               <p className="text-sm text-center px-4 max-w-md mt-2 text-zinc-500">
                 {searchMode === 'keyword' 
                   ...
                   : "Select a resume and we'll automatically generate the best search query for you."}
               </p>
          ...
          {/* Search Results List (Used for Keyword, Resume AND Manual Result) */}
          {activeTab === 'search' && jobs.length > 0 && (
            <div className="max-w-4xl mx-auto space-y-4 md:space-y-6 animate-in fade-in slide-in-from-bottom-4">
               ...
               {jobs
                 .sort((a, b) => (b.analysis?.matchScore || 0) - (a.analysis?.matchScore || 0)) // Sort by match score
                 .map((job) => {
